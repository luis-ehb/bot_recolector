...
# --- Main ---
def main():
    debug_img_path = os.path.join(os.path.dirname(__file__), "debug_screenshot.png")
    img = cv2.imread(debug_img_path)
    if img is None:
        print(f"[ERROR] No se pudo cargar la imagen {debug_img_path}")
        return
    ...
    print(f"Detectados: {[d['label'] for d in detections]}")

    debug_output_path = os.path.join(os.path.dirname(__file__), "debug_templates.png")
    cv2.imwrite(debug_output_path, img)
    print(f"[INFO] Imagen de detecciones guardada en {debug_output_path}")
